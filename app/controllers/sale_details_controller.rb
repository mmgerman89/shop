class SaleDetailsController < ApplicationController
  before_action :set_sale, only: %i[new create destroy]
  ...
  def create
    ...
    @sale.sale_details.each do |detail|
      next unless detail.item_id == item_id
    ...
    if item_exists
      ...
      sale_detail.number = if @sale.sale_details.last.nil?
                             1
                           else
                             @sale.sale_details.last.number + 1
                           end
